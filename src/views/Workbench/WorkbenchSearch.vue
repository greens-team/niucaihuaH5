  ...
  <div class="WorkbenchSearch flex-1 flex flex-col">
    ...
    <div class="flex items-center pr-2 pl-3">
      ...
      <van-search
        ...
        shape="round"
      ></van-search>
      <div slot="action" @click="$router.go(-1)" style="color:#323233">取消</div>
    </div>

    <van-tabs class="-mt-2" v-model="params.modelType">
      <van-tab v-for="(row,index) in tabs" :key="index" :title="row.name" :name="row.id"></van-tab>
    </van-tabs>
...
.WorkbenchSearch /deep/ .van-hairline--top-bottom::after,
.WorkbenchSearch /deep/ .van-hairline-unset--top-bottom::after {
  border: 0;
}
.WorkbenchSearch /deep/ .van-tab--active span {
  font-size: 1.2rem;
}

...
export default {
  ...
  watch: {
    '$store.state.competitor.listParams.queryString'(val){
      this.$store.dispatch('listCompetitor', {queryString: val, isOnlyWrite:this.$route.query.isOnlyWrite ? true : false})
    }
  },
  mounted () {
    ...
    this.$store.dispatch('listCompetitor',{isOnlyWrite:this.$route.query.isOnlyWrite ? true : false})
  },
  methods: {
    selectedList(){

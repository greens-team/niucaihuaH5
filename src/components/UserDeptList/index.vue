...
<div class="userDeptList flex-1 flex flex-col fixed inset-0 z-50">
  <div class="items-center pl-4 pr-4 flex border-b border-gray-200 bg-white">
      ...
      <span class="text-center font-bold bar_title" style="font-size: 1.286rem;">选择同事</span>
    ...
    <div class="bg-gray-100 p-1 pt-3 pb-3 pl-5 flex items-center">
      <div>已选择：</div>
      <div @click="showResults=true" class="ellipsis flex-1 pr-2" v-if="[].concat.apply([],tabs.filter((r,i)=>i && r).map(r=>r.values)).length">{{[].concat.apply([],tabs.filter((r,i)=>i && r).map(r=>r.values)).map(r=>r.split('_')[0]).toString()}}</div>
      ...
      <div class="sendBtn ml-1" style="width:61px; height:39px;"  @click="getResults">确定</div>
    </div>
        ...
        changeCheck(row, values){
          ...
          if(values.includes(row.value)){
            ...
            if(index == 2){
              ...
              // 选中当前组织时，删除同组织上的其它数据，但不包括兄弟组织
              var nodes = []; // 获取到当前树
              var forChildren = function(data){
                  return data.some(r=>{
                    if(r.value == row.value.substr(2)){
                        r.level == 1 ? (nodes = [r.children]) : nodes.splice(r.level-1,1,r.children)
                        return true   
                    }else{
                        if(r.children){
                            nodes.splice(r.level-1,1,r.value)
                            return forChildren(r.children)
                        }
                      }
                  })
              }
              forChildren(self.tabs[index].data[0].children)
              // 遍历当前树 清除
              nodes.map(r=>{
                if(typeof r == 'string' && values.includes("2_"+r)){
                  values.splice(values.indexOf("2_"+r), 1)
                  self.tabs[index].values.splice(self.tabs[index].values.indexOf(r), 1)
                }
                if(r instanceof Array){
                  // 删除该部门下已经选中的子数据
                  let eachVal = function(arr){
                      arr.map(r=>{
                          let i = values.indexOf('2_'+r.value)
                          if(i != -1){
                              values.splice(i, 1)
                              self.tabs[index].values.includes(r.value) && self.tabs[index].values.splice(self.tabs[index].values.indexOf(r.value), 1)
                          }
                          r.children && r.children.length && eachVal(r.children)
                      })
                  }
                  eachVal(r)
                }
      ...
      },
      {
        ...
        changeCheck(row, values){
          if(values.includes(row.value)){
            ...
            let eachVal = function(arr){
                arr.map(r=>{
                    let index = values.indexOf(r.value)
                    if(index != -1){
                        values.splice(index, 1)
                        self.tabs[0].values.includes('2_'+r.value) && self.tabs[0].values.splice(self.tabs[0].values.indexOf('2_' + r.value), 1)
                    }
                    r.children && eachVal(r.children)
                })
            }
            row.children && row.children.length && eachVal(row.children)
          }
        }
      }
      ],
    ...
    getTabsData(index){
      ...
      this.$store
        ...
        .then((data) => {
          ...
          if(index == 2){
            let forArr = function(data, num){
              data.map(r=>{
                r.level = num
                r.children && forArr(r.children, num+1)
              })
            }
            forArr(this.tabs[index].data, 0)
...
.Workbench .userDeptList /deep/ .van-tab span {
  font-size: 1rem;
}

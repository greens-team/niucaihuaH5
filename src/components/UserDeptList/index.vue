      ...
      <div class="absolute inset-0">
        <van-swipe ref="swipe" :loop="false" :show-indicators="false" @change="(num)=>{active=num}">
          <van-swipe-item
            v-for="(tab,i) in tabs"
            :key="i"
            class="pt-2 overflow-y-scroll colleagueListBox"
          >
            <van-search
              v-show="tab.search"
              class="-mt-2"
              shape="round"
              :placeholder="'请输入搜索关键词'+tab.name"
              v-model="searchKeyword"
            />

            <!-- 面包屑 -->
            <div
              v-if="tab.tree && tab.data.length > 1"
              class="flex ml-5 mr-5 border-b border-gray-200 pb-2"
            >
              <div v-for="(row,i) in tab.data" :key="i" class="text-blue-600">
                <span class="text-xs pl-1 pr-1 text-gray-500" v-if="i">></span>
                <span
                  @click="tab.data.splice(i+1,tab.data.length)"
                  :class="['text-sm',{'text-gray-500': i == tab.data.length-1}]"
                >{{row.lable}}</span>
              </div>
            </div>
            ...
            <van-checkbox-group
              v-model="tab.values"
              :class="['bg-white checkBoxGroup', {treeImg: tab.tree}]"
            >
              <van-checkbox
              ...
              >
                <div v-if="tab.tree" class="w-full flex items-center">
                  ...
                  <div v-if="row.children && row.children.length">
                    <div
                      v-if="tab.values.includes(row.value)"
                    ...
                    <div
                      v-else
                      @click="(e)=>{tab.data.push(row);nextLevel(e, tab.data);}"
                      class="flex items-center"
                    >
        ...
        <template v-if="tabs[2] && tabs[2].tree">
          <div v-for="(row, i) in tabs[2].values" :key="'a'+i">
            <div class="flex items-center pt-3 pb-3 ml-5 mr-5 border-b border-gray-200">
              ...
              <div class="flex-1 ml-2">{{row.split('_')[0]}}</div>
...
export default {
      ...
      tabs: [
        {
          ...
          changeCheck(row, values) {
            ...
            if (values.includes(row.value)) {
              self.tabs[index].values.splice(
                self.tabs[index].values.indexOf(row.value.substr(2)),
                1
              );
            } else {
        ...
        },
        {
          ...
          changeCheck(row, values) {
            if (values.includes(row.value)) {
              self.tabs[0].values.includes("1_" + row.value) &&
                self.tabs[0].values.splice(
                  self.tabs[0].values.indexOf("1_" + row.value),
                  1
                );
            } else {
        ...
        },
        {
          ...
          changeCheck(row, values) {
            if (values.includes(row.value)) {
              self.tabs[0].values.includes("2_" + row.value) &&
                self.tabs[0].values.splice(
                  self.tabs[0].values.indexOf("2_" + row.value),
                  1
                );
            } else {
              self.tabs[0].data.some(r => {
  ...
  watch: {
    ...
    active(index) {
      ...
      this.scrollLoadData(
        document.getElementsByClassName("colleagueListBox")[1],
        resolve => {
          if (!this.colleagueLastPage) {
            ...
            this.$store
              .dispatch(
                this.tabs[index].url,
                Object.assign(this.tabs[index].getParams(this.searchKeyword), {
              ...
              .then(data => {
                !data && (data = []);
                this.tabs[index].data = this.tabs[index].data.concat(
                  this.tabs[index].filterData
                    ? this.tabs[index].filterData(data)
                    : data
                );
  ...
  methods: {
    deleteRow(rowIndex, index) {
      let resIndex = this.tabs[0].values.indexOf(
        rowIndex + "_" + this.tabs[rowIndex].values[index]
      );
    ...
    scrollLoadData(domBox, callback, positionCallback = false) {
      ...
      domBox.onscroll = function() {
        ...
        if (
          domBox.scrollTop > domBox.scrollHeight - domBox.clientHeight - 10 &&
          !isSend
        ) {
          isSend = true;
          new Promise(resolve => {
    ...
    getTabsData(index) {
      ...
      this.$store.dispatch(this.tabs[index].url, params).then(data => {
        this.tabs[index].data = this.tabs[index].filterData
          ? this.tabs[index].filterData(data)
          : data;
      });
    },
    ...
    getResults() {
      this.$emit(
        "confirm",
        this.tabs[1].values,
        this.tabs[2] ? this.tabs[2].values : []
      );
    }
  }
};
...
.treeImg /deep/ .van-checkbox__label {
  width: 100%;
  padding-right: 5px;
}

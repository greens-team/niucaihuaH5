...
<template>
  <div class="ContactsList flex flex-1 flex-col">
    ...
    <div class="flex bg-gray-100 mt-1 p-3 h-14 items-center" v-show="$store.state.contacts.selectedUserGids.length">
  ...
  methods: {
    selectedList(){
      if(this.$store.state.contacts.selectedUserGids.length){
        this.$store.state.contacts.jobsUser= [];
        this.$route.query.modelGid ? this.$router.push({path:'/SelectedList', query: {modelGid: this.$route.query.modelGid}}) : this.$router.push('/SelectedList') ;
